// ============================================================
//  SI-DEVA — Auth UI & Role Management v3.0
...
(function injectAuthStyle() {
  ...
  s.textContent = `
    ...
    /* Hapus duplikat di index.html integration patch jika ada */
...
function showAuthOverlay() {
  ...
  overlay.querySelectorAll('.auth-input').forEach(el => {
    el.addEventListener('keydown', e => { if (e.key === 'Enter') doAuthLogin(); });
  });
...
async function doAuthLogin() {
  ...
  try {
    ...
    toast(`☁️ Selamat datang! Role: ${getRole()}`, 'success');
  } catch(err) {
    errEl.textContent = err.message.includes('Email not confirmed')
...
async function doAuthRegister() {
  ...
  try {
    ...
    errEl.textContent = '✅ Akun dibuat! Silakan login. (Konfirmasi email jika diperlukan)';
  } catch(err) {
...
// ── Role UI: sembunyikan tombol sesuai role ───────────────────
function applyRoleUI() {
  const role = getRole();

  // Update badge role di sidebar/topbar
  _updateRoleBadge(role);

  // Viewer: sembunyikan semua tombol Tambah, Edit, Hapus
  const writeActions = document.querySelectorAll(
    'button[onclick*="openAddModal"], button[onclick*="editRecord"], button[onclick*="deleteRecord"], ' +
    'button[onclick*="savePaket"], button[onclick*="saveRincian"], button[onclick*="saveHarga"], ' +
    'button[onclick*="savePenyedia"], .btn-tambah, #btn-tambah'
  );
  ...
  const masterActions = document.querySelectorAll(
    'button[onclick*="saveBidang"], button[onclick*="saveOpd"], button[onclick*="saveRekening"], ' +
    'button[onclick*="savePPK"], button[onclick*="savePejabatPengadaan"], button[onclick*="saveEcatalog"]'
  );
  masterActions.forEach(btn => { btn.style.display = isAdmin() ? '' : 'none'; });

  // Import & Reset: hanya admin
  const adminOnly = document.querySelectorAll(
    'button[onclick*="importCSV"], button[onclick*="importJSON"], button[onclick*="resetDB"], ' +
    'button[onclick*="loadSampleData"], #page-import, .import-section'
  );
  adminOnly.forEach(el => { el.style.display = isAdmin() ? '' : 'none'; });

  // Menu Import di sidebar: sembunyikan untuk viewer
  document.querySelectorAll('.nav-item').forEach(btn => {
    const oc = btn.getAttribute('onclick') || '';
    if (oc.includes("'import'") && !isAdmin()) btn.style.display = 'none';
  });

  // Panel manajemen user: hanya admin
  const mgmtPanel = document.getElementById('user-mgmt-panel');
  if (mgmtPanel) mgmtPanel.style.display = isAdmin() ? '' : 'none';

  // Semua menu/elemen admin-only: hanya admin
  // PENTING: gunakan classList, BUKAN inline style — agar tidak konflik dengan CSS inject
  document.querySelectorAll('.admin-only').forEach(el => {
    if (isAdmin()) {
      el.classList.add('sideva-admin-show');
      el.classList.add('sideva-visible');     // kompatibel dengan style.css
    } else {
      el.classList.remove('sideva-admin-show');
      el.classList.remove('sideva-visible');
    }
  });

  // Tombol logout sidebar hanya tampil saat session aktif
  document.querySelectorAll('.sidebar-logout').forEach(el => {
    el.style.display = isLoggedIn() ? '' : 'none';
  });

  // Tombol Kirim/Ambil cloud di halaman backup: hanya admin
  const cloudBtns = document.querySelectorAll('#sync-loggedin-area button');
  if (!isAdmin()) cloudBtns.forEach(b => { if (!b.onclick?.toString().includes('Logout')) b.style.display = 'none'; });
}
...
function _updateRoleBadge(role) {
  ...
  badge.id = 'role-badge';
  const labels = { super_admin: '🔱 Super Admin', admin: '👑 Admin', operator: '✏️ Operator', viewer: '👁️ Viewer' };
  badge.textContent = labels[role] || role;
  badge.className   = `role-badge role-${role}`;

  // Tambahkan ke sidebar footer
  const footer = document.querySelector('.sidebar-footer');
  if (footer) footer.prepend(badge);
}

// ── Panel Manajemen User (di halaman Backup, hanya admin) ─────
            ...
            <tr>
              ...
              <td>
                <select class="role-select" onchange="changeUserRole('${u.user_id}', this.value)">
                  ...
                  <option value="admin"    ${u.role==='admin'    ? 'selected':''} ${!isSuperAdmin() ? 'disabled' : ''}>👑 Admin${!isSuperAdmin() ? ' (butuh Super Admin)' : ''}</option>
                  <option value="operator" ${u.role==='operator' ? 'selected':''}>✏️ Operator</option>
                  <option value="viewer"   ${u.role==='viewer'   ? 'selected':''}>👁️ Viewer</option>
                </select>
              </td>
              <td style="font-size:11px;opacity:.4;">${new Date(u.created_at).toLocaleDateString('id-ID')}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
...
async function changeUserRole(userId, newRole) {
  try {
    ...
    toast('Gagal ubah role: ' + err.message, 'error');
    renderUserMgmt(); // reset dropdown
  }
}

// ── Inject panel auth ke halaman Backup ──────────────────────
  ...
  if (!isLoggedIn()) {
    mount.innerHTML = `
      <div style="text-align:center;padding:20px 0;">
        <p style="opacity:.5;font-size:13px;margin-bottom:12px;">Belum login ke cloud.</p>
        <button class="btn btn-primary" onclick="showAuthOverlay()">🔐 Masuk ke Cloud</button>
      </div>
    `;
    return;
  }

  const roleLabels = { super_admin: '🔱 Super Admin', admin: '👑 Admin', operator: '✏️ Operator', viewer: '👁️ Viewer' };
  mount.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;flex-wrap:wrap;">
      <span style="font-size:13px;font-weight:600;color:#4ade80;">☁️ Cloud: Aktif</span>
      <span style="font-size:12px;opacity:.5;">${user?.email || ''}</span>
      <span class="role-badge role-${role}" style="padding:2px 10px;border-radius:20px;font-size:11px;font-weight:700;">${roleLabels[role]||role}</span>
    </div>
    ${isAdmin() ? `
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px;">
      <button class="btn btn-secondary" style="font-size:13px;" onclick="loadAllData().then(()=>{if(typeof renderAll==='function')renderAll();toast('Data diperbarui','success')})">🔄 Refresh Data</button>
      <button class="btn btn-danger"    style="font-size:13px;" onclick="doCloudLogout()">Keluar</button>
    </div>
    <div style="margin-bottom:8px;font-size:12px;font-weight:600;opacity:.7;">👥 Manajemen User</div>
    <div id="user-mgmt-panel"></div>
    ` : `
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
      <button class="btn btn-secondary" style="font-size:13px;" onclick="loadAllData().then(()=>{if(typeof renderAll==='function')renderAll();toast('Data diperbarui','success')})">🔄 Refresh Data</button>
      <button class="btn btn-danger"    style="font-size:13px;" onclick="doCloudLogout()">Keluar</button>
    </div>
    <p style="font-size:11px;opacity:.4;margin:8px 0 0;">Data otomatis diperbarui setiap 30 detik.</p>
    `}
  `;
...
async function doCloudLogout() {
  if (typeof logAudit === 'function') await logAudit('logout', {});
  await sbLogout();
  applyRoleUI();
  injectAuthPanel();
  if (typeof renderAll === 'function') renderAll();
  toast('Keluar dari cloud.', 'info');
  showAuthOverlay();
}
...
window.addEventListener('sideva:page-changed', (e) => {
  ...
  setTimeout(() => { injectAuthPanel(); if (isAdmin()) renderUserMgmt(); }, 50);
});
